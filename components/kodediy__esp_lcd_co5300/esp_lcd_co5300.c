...
typedef struct
{
    ...
    struct
    {
        ...
        unsigned int use_spi_interface : 1;  // Set to 1 if use SPI interface, 0 for QSPI
    } flags;
} co5300_panel_t;
...
esp_err_t esp_lcd_new_panel_co5300(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    if (vendor_config)
    {
        ...
        co5300->flags.use_spi_interface = vendor_config->flags.use_spi_interface;
    }
...
{
    if (co5300->flags.use_spi_interface) {
        // For SPI mode, use standard API without opcode bit manipulation
        return esp_lcd_panel_io_tx_param(io, lcd_cmd, param, param_size);
    } else {
        // For QSPI mode, use opcode bit manipulation
        lcd_cmd &= 0xff;
        lcd_cmd <<= 8;
...
esp_err_t esp_lcd_panel_co5300_set_brightness(esp_lcd_panel_handle_t panel, uint8_t brightness)
{
    ...
    ESP_RETURN_ON_FALSE(brightness <= 100, ESP_ERR_INVALID_ARG, TAG, "brightness value out of range");
